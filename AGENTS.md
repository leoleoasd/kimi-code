...
- Remote-control surfaces in `apps/kimi-code`: the `kimi remote connect` CLI subcommand (`src/cli/sub/remote/`), the TUI `/remote connect|status|disconnect` slash commands (`src/tui/commands/remote.ts`, `remote-bridge.ts`), and the `kimi headless` subcommand (`src/cli/sub/headless/`) — a terminal-less background agent (detached respawn, `--foreground` to attach): API-only loopback kap-server + one fresh (`--session <id>` to attach an existing one) cwd session + the same tunnel/notify/hub-tools wiring as `remote connect`, exposed only through the hub web UI. Headless injects its own small command bridge (`commands.ts`: `/title`, `/help`) and takes the full process exit on the kap-server `/api/v1/shutdown` route (wired through `runServerInProcess`'s `shutdownHandler`), which the hub web exposes as a two-tap "Stop agent" button on pid-carrying agents. TUI attach scopes the live session without a restart.
- kap-server host-injection seams (fork-updated `packages/kap-server/AGENTS.md` has the details): an embedding host (the TUI's `/remote connect`) passes a live engine via `ServerStartOptions.core`; `ServerStartOptions.commandBridge` (`src/transport/commandBridge.ts`) exposes the host's slash-command grammar remotely — `POST /sessions/{id}:command` runs a raw line, `GET /sessions/{id}/commands` powers composer hints; unbridged servers answer `40421 command.unavailable` + an empty catalog.
...
- `packages/agent-core-v2`: the DI × Scope agent engine (the v2 port behind kap-server). Four `LifecycleScope` tiers — `App` / `Workspace` / `Session` / `Agent` (`app/scopes.ts`) — plus the L3 unit layer (`Service`/`Fiber` units, collection contribution points, the Feature seam in `src/features/`); session lifecycle goes through the App-scope `ISessionManager` (create/resume/fork/close → `ISessionScopeHandle`), and workspaces materialize on demand through the App-scope `IWorkspaceInstanceManager` (`getOrCreate` → `WorkspaceInstance.program`, which holds the workspace-scoped service generation). See `packages/agent-core-v2/AGENTS.md` and use the `agent-core-dev` skill (`.agents/skills/agent-core-dev/SKILL.md`) when developing here.
...
- **Fork distribution = GitHub Releases on `leoleoasd/kimi-code`.** The "Fork Release" workflow (`.github/workflows/fork-release.yml`) triggers on `workflow_dispatch` (pick a tag) or by pushing a tag matching `fork-v*`; it builds the CLI native bundles (linux/darwin/win32 × x64/arm64 via the reusable `_native-build.yml`) and the `kimi-hub` SEA executables (linux/darwin × x64/arm64, with the hub web UI embedded), then publishes zips + sha256s, bare executables + sha256s, and a `manifest.json` (version + per-platform filename/checksum, upstream CDN schema shape) as release assets. Keep fork releases marked non-prerelease: `install.sh` resolves the latest non-prerelease release.
- Fork-channel self-update: `_native-build.yml` stamps the tag version into the binary via `KIMI_CODE_FORK_VERSION` (`apps/kimi-code/tsdown.native.config.ts` define `__KIMI_CODE_FORK_VERSION__` → `KIMI_BUILD_INFO.forkVersion`). A stamped fork binary checks `releases/latest/download/manifest.json`, stages `<release>/manifest.json` + the bare binary (`<repo>/releases/download/fork-v<version>/…`), and swaps it in at next startup — the same machinery as upstream, with staged payloads cross-checked by channel so fork and upstream artifacts never replace each other. `install:local` builds pass no stamp and never self-update.
- `install.sh` (repo root, raw.githubusercontent URL on the fork's `main`) installs `kimi` + `kimi-hub` from those releases into `~/.kimi-code/bin` — same layout as the official installer, manifest-driven with sha256 verification, existing binaries backed up to `.bak`, and the bin dir appended to the shell rc (`--cli-only` / `--hub-only`, `--install-dir`, `--version`; env `KIMI_FORK_REPO`, `KIMI_INSTALL_DIR`, `KIMI_NO_MODIFY_PATH`, `KIMI_FORK_RELEASES_BASE_URL`). Windows users download the `win32-*` archive manually.
