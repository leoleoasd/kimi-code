...
- Cross-session messaging over the hub: while a remote-control connection is up, agents gain `ListHubSessions` + `SendHubMessage` — the hub roster read and a wrapped user-role prompt delivery to another session, both via plain HTTPS with the shared token (the tunnel protocol has no agent→hub channel). Engine keeps the connection in App-scope `IHubConnectionService` (`packages/agent-core-v2/src/hub/`); the connectors register the tools per agent directly (`apps/kimi-code/src/cli/sub/remote/shared.ts` `wireHubTools`), so they vanish when the link drops.
