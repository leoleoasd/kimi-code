...
describe('Agent loop', () => {
  ...
  it('fails the turn after a filtered step completes', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [wire] llm.request                 { "kind": "loop", "provider": "openai", "model": "mock-model", "modelAlias": "mock-model", "thinkingEffort": "off", "maxTokens": 1000000, "toolSelect": false, "systemPromptHash": "ec9c34379c88babbc468ef2f3e0e08cd2f422c8c4a910664fb8bb394d703a575", "toolsHash": "2a746f38b201227622027e8840d7865378ca4c97177701dc52628e969ffec810", "messageCount": 1, "turnStep": "0.1", "time": "<time>" }
  ...
  it('carries media content parts on turn.started; omits content for text-only turns', async () => {
    const bus = ctx.get(IEventBus);
    const starts: Array<{ prompt?: string; content?: unknown }> = [];
    const subscription = bus.subscribe('turn.started', (e) => {
      starts.push({ prompt: e.prompt, content: e.content });
    });

    ctx.mockNextResponse({ type: 'text', text: '<text-1>' });
    await ctx.rpc.prompt({
      input: [
    ...
    await ctx.untilTurnEnd();
    ctx.mockNextResponse({ type: 'text', text: '<text-2>' });
    await ctx.rpc.prompt({ input: [{ type: 'text', text: 'plain' }] });
    await ctx.untilTurnEnd();
    subscription.dispose();
    ...
    expect(starts).toEqual([
      ...
      // Text-only turns keep the event lean (attachment projections have no
      // parts to consume anyway).
      { prompt: 'plain', content: undefined },
    ]);
  });
});
