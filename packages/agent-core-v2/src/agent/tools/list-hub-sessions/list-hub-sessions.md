List the sessions reachable on the kimi hub this session is attached to: one block per connected agent (machine name, platform, working directory) with the session ids it exposes, each shown with its title when one is set.
