...
The cold rebuild is a two-level fold over `wire.jsonl` as the single source of truth: `history/groupTurns.ts` (context messages → turn tree) plus `history/foldFacts.ts` (non-context records → tasks, interactions, todos, goal/plan/swarm meta, and turn-anchored markers/taskrefs; interactions left pending at shutdown fold to `cancelled`). Standalone items sit between turns: plan markers anchor BEFORE the turn they were recorded in (they describe the plan that turn is about to execute), all other mid-turn markers/taskrefs anchor after it. Media on a turn-opening user message folds into attachment entities from BOTH persisted vocabularies — the legacy v1 `image`/`video`/`file` + `source` shapes (keeping their `url`/`file` sources) and the v2 core `image_url`/`video_url` parts (camelCase inner keys): `data:`/`http(s)` → `url`, `blobref:<mime>;<sha256>` → the `blob` source (`ref` = the full blobref string; bytes live in the agent-scoped blob store, served by kap-server's blob route), and `kimi-file://<fileId>` urls fall through to the daemon file ref (`contract/mediaRef.ts`) → the `session_media` source. All url classification is pure string ops — the package never imports the engine.
