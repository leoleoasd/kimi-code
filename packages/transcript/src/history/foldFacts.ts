 ...
 *  - markers and taskrefs are anchored to the ENGINE turn open at their
 *    record position (the fold replays the turn clock: turn.prompt records
 *    assign ids, interruptions anchor to their own cancelled id) and placed
 *    AFTER that turn's item — falling back to record order at the end when
 *    the owning turn has no item (e.g. its messages were compacted out).
 *    Timestamps cannot be compared against the base items (turns carry no
 *    `at`), so this is a turn-boundary-anchored, not time-anchored, placement;
 *    within one turn several facts land in record order.
...
/**
 * Insert turn-anchored fact markers/taskrefs into the base items: a marker
 * lands AFTER its owning turn's item; when that turn has no item (its
 * messages were compacted away, or its ordinal sits between two survivors)
 * the marker is placed BEFORE the next surviving turn instead. Leftovers
 * (anchors past the last turn, or no turns at all) keep record order at the
 * end. Same-anchor entries stay in record order.
 */
function placeAnchored(
  items: readonly TranscriptItem[],
  appended: readonly { readonly item: TranscriptItem; readonly after: number }[],
): readonly TranscriptItem[] {
  if (appended.length === 0) return items;
...
export function foldWireRecordFacts(
  ...
  /**
   * Anchor placement is only meaningful once the journal carries a real turn
   * clock (any `turn.prompt` record); older journals without one keep the
   * legacy "facts append in record order at the end" placement.
   */
  let sawTurnClock = false;

  /**
   * Markers/taskrefs generated by the fold, each anchored to the engine turn
   * open at its record position (`after`): inserted after that turn's item at
   * merge time instead of piling at the end of the snapshot. Interruptions
   * anchor to the cancelled turn itself.
   */
  const appended: { readonly item: TranscriptItem; readonly after: number }[] = [];
  const activeCancelTurnIds = new Set<number>();
  /** Engine turn id open at the current record position (clamped ≥ 0). */
  ...
  const pushMarker = (marker: string, record: HistoryWireRecord, after?: number): void => {
    markerSeq += 1;
    ...
    appended.push({ item, after: after ?? currentTurnAnchor() });
  };
  ...
  const upsertTask = (record: HistoryWireRecord): void => {
    ...
    if (record.type === 'task.started') {
      ...
      if (!usedRefIds.has(refId)) {
        ...
        appended.push({ item: ref, after: currentTurnAnchor() });
      }
    }
  };
  ...
  return {
    ...
    items:
      appended.length === 0
        ? items
        : sawTurnClock
          ? placeAnchored(items, appended)
          : [...items, ...appended.map((a) => a.item)],
    tasks: [...tasks.values()],
