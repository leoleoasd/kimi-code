 ...
 *
 * The submission's `goal_objective` / `goal_control` fields dispatch onto the
 * target agent's `IAgentGoalService` with the same semantics as the profile
 * edge's `agent_config` translation (`sessionAgentConfig.ts`): objective before
 * control, and engine validation / lifecycle conflicts surface verbatim. An
 * objective rides the submitted prompt (the objective text is the turn that
 * starts the goal); a control-only submission launches no turn for its
 * `content` (a control placeholder is never a user message), so no prompt is
 * minted — the reply stays `PromptSubmitResult`-shaped with
 * `status: 'blocked'`, the wire's "rejected before a turn is launched" signal.
 */
  ...
  if (isError2(err)) {
    switch (err.code) {
      ...
      case ErrorCodes.GOAL_OBJECTIVE_TOO_LONG:
        ...
        return;
      case ErrorCodes.GOAL_UNSUPPORTED_AGENT:
        reply.send(
