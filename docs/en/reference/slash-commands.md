...
| `/remote connect [<hub-url>] [--token <t>] [--name <n>]` | — | Bridge the current session to a running kimi hub without leaving the TUI — the hub's web UI and the terminal drive the same live session; an omitted `<hub-url>` targets the local hub (`http://127.0.0.1:58630`). `/remote status` shows the link, `/remote disconnect` tears it down. See [`kimi remote connect`](./kimi-command.md#kimi-remote-connect) | No |
